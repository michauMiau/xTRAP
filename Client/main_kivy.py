...
# Patch MTDev import before Kivy loads it (suppresses crash from missing libmtdev)
try:
    class FakeMtdev:
        Device = None
    
    import sys
    sys.modules['kivy.input.providers.mtdev'] = FakeMtdev()
except Exception:
    pass
...
from network import network_loop, send_steering
...
class IPDisplay(FloatLayout):
    """IP display panel — placeholder for now"""
    
    def __init__(self):
        super().__init__()
        
        # Will add real IP management later (TextInput + connect button)
        self.ip_label = Label(
            text="IP: 192.168.1.174", 
            font_size=16, 
            size_hint=(1.0, None), 
            height=30
        )
...
class RCControlCenterApp(App):
    ...
    def build(self):
        ...
        self.ip_display = IPDisplay()
        self.ip_display.pos_hint = {"bottom": 0}
        root.add_widget(self.ip_display)
        
        # Bind buttons to send commands
        self.steering_panel.left_btn.bind(on_press=lambda *a: self.send_steering(0))   # Full left
        self.steering_panel.right_btn.bind(on_press=lambda *a: self.send_steering(180))  # Full right
